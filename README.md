...
Расширение Chrome для селлеров Kaspi.kz: реальная маржа по товару (комиссии
Kaspi 2026 + налоги РК) и анти-демпинг конкурентов прямо на карточке. Бесплатно
для калькулятора и 3 товаров под наблюдением, дальше Pro. Локально, без регистрации.

**Закрытый альфа-тест, версия 0.1.0-alpha.9.** Manifest V3 + TypeScript strict + React 19.

📄 [Политика приватности](https://yerkebulan-ardabayev.github.io/Tirek-Extension/privacy.html)

## Для тестеров → [tirek landing-page](https://yerkebulan-ardabayev.github.io/Tirek-Extension/)

Скачайте zip, распакуйте, Load Unpacked в Chrome — 4 шага, 30 секунд.

📦 **Последний релиз:** [v0.1.0-alpha.9](https://github.com/Yerkebulan-Ardabayev/Tirek-Extension/releases/latest)

🐛 **Баги/предложения:** [Issues](https://github.com/Yerkebulan-Ardabayev/Tirek-Extension/issues)
...
- 🛡 **Tirek: N демперов, X%**: есть конкуренты ниже порога демпинга (по умолчанию -5%)
- ✅ **Tirek: вы дешевле всех**: никто не стоит ниже вашей цены
- ℹ **Tirek: демперов нет, ниже вас: N**: демперов нет, но N продавцов чуть дешевле вас
...
> **Запланировано, в текущей сборке (0.1.0-alpha.9) НЕ включено.**
> Парсер кабинета `/mc/*` отключён и не поставляется: его DOM-селекторы
> ещё не проверены на реальном кабинете. Инжектится только
...
### 3. Калькулятор маржи (popup → вкладка «Маржа», открыта по умолчанию)
...
- Когда вы открываете карточку отслеживаемого товара, Tirek сверяет цены и при новом демпере шлёт push-уведомление Chrome:
  - 🛡 «Tirek: новый демпер»
...
4 вкладки (по умолчанию открывается «Маржа» — главный сценарий):
- **Маржа** — калькулятор реальной прибыли, см. п. 3
- **Демперы** — KPI: новых демперов 24ч, под наблюдением, средняя маржа
- **Наблюдение** — список SKU + быстрое редактирование себестоимости
- **Настройки** — налоговый режим, СПП/Red, порог демпинга, имя магазина, тариф

### 6. Тариф

- **Бесплатно:** калькулятор маржи без ограничений + наблюдение за 3 товарами.
- **Pro — 2990 ₸/мес:** безлимит товаров под наблюдением и анти-демпинг по всему портфелю.

Оплата вне расширения (перевод через Kaspi), активация по коду в «Настройки» →
«Тариф». Код подписан офлайн (ECDSA P-256) и привязан к ID установки: подделать
или расшарить нельзя, приватный ключ в расширение не входит (см. `src/lib/license.ts`).
...
(`[Tirek/parser] price from .item__price-once → 25990`). При поломке открой
